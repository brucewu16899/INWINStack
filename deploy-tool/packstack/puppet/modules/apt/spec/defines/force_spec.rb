...
describe 'apt::force', :type => :define do
  ...
  describe "when using default parameters" do
    ...
    it { should contain_exec("sudo /usr/bin/apt-get -y -t #{params[:release]} install #{title}").with(
  ...
  describe "when specifying false release parameter" do
    ...
    it { should contain_exec("sudo /usr/bin/apt-get -y  install #{title}").with(
  ...
  describe "when specifying version parameter" do
    ...
    it { should contain_exec("sudo /usr/bin/apt-get -y -t #{params[:release]} install #{title}=#{params[:version]}").with(
      :unless => "sudo /usr/bin/apt-cache policy -t #{params[:release]} #{title} | /bin/grep -q 'Installed: #{params[:version]}'"
    ) }
  end
  ...
  describe "when specifying false release and version parameters" do
    ...
    it { should contain_exec("sudo /usr/bin/apt-get -y  install #{title}=1").with(
