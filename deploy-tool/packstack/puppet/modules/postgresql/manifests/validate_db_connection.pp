...
define postgresql::validate_db_connection(
  ...
  # cylee : 2014-05-20 : force it run as postgres
  $cmd_init = "${psql_path} --tuples-only --quiet -S "
  ...
  #$validate_cmd = "/usr/local/bin/validate_postgresql_connection.sh ${sleep} ${tries} '${cmd}' || true"
  ...
  exec { $exec_name:
    ...
    # cylee : their verify function have bug it will stuck forever
    #unless      => $validate_cmd,
    unless      => "/usr/bin/sudo -u ${run_as} /usr/bin/psql --tuples-only --quiet -S --dbname postgres -c 'SELECT 1;'",
    cwd         => '/tmp',
    #environment => $env,
